...
class AIAssistantGemini(override val properties: Properties) : AIAssistant {
    ...
    override fun buildRequest(prompt: String): Request {
        ...
        // Build the generationConfig JSON block using config properties.
        // Google's Gemini API expects "maxOutputTokens" for max tokens constraint.
        val generationConfig = JSONObject()
            .put("temperature", temperature)         // Sourced from AIAssistant.temperature -> config.properties TEMPERATURE
            .put("maxOutputTokens", maxTokens)       // Sourced from AIAssistant.maxTokens -> config.properties MAX_TOKENS

        // Build the complete request body with model selection, content, and generationConfig
