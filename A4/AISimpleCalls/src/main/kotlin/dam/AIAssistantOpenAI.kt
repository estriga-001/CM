...
class AIAssistantOpenAI(override val properties: Properties) : AIAssistant {
    ...
    override fun buildRequest(prompt: String): Request {
        ...
        // Build the complete request body with model selection, messages, and config-driven parameters.
        // `temperature` and `max_tokens` are read dynamically from AIAssistant properties.
        val requestBody = JSONObject()
            .put("model", model)              // Specify which model to use
            .put("messages", messagesArray)   // Conversation history
            .put("temperature", temperature)  // Randomness controls: from config.properties TEMPERATURE
            .put("max_tokens", maxTokens)     // Response cap: from config.properties MAX_TOKENS
            .toString()                       // Convert to standard JSON string
    ...
    override fun parseResponse(responseBody: String): String {
        ...
        // OpenAI uses "choices" -> 0 -> "message" -> "content"
